...
export default function QuanLyKhachHang() {
  ...
  useEffect(async () => {
    try {
      ...
      const data = await (
        await axios.get("http://localhost:8888/api/khach-hang")
      ).data.data;
      // let data = [
      //   {
      //     id_khach_hang:"01",
...
export function KhachHangMoi(khachHangTuBang) {
  const [id_khach_hang, setMaKhachHang] = useState(
    khachHangTuBang.khachHangmoi.id_khach_hang
  );
  const [ten_khach_hang, setTenKhachHang] = useState(
    khachHangTuBang.khachHangmoi.ten_khach_hang
  );
  const [gioi_tinh, setGioiTinh] = useState(
    khachHangTuBang.khachHangmoi.gioi_tinh
  );
  const [so_cmnd, setSoChungMinhNhanDan] = useState(
    khachHangTuBang.khachHangmoi.so_cmnd
  );
